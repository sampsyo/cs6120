...
We expanded Bril's interpreter to handle the five new op codes described above, its implementation was relatively straightforward for break and continue statements and [more invovled for the rest of the operations](https://github.com/gerardogtn/bril/blob/f2f795c4a313596510772a9c24d4697ccf9904c9/brili.ts#L878). In particular, correctly implementing multi-level `break`s and `continue`s required careful tracking of a `level` counter to track how many loop boundaries a statement needs to cross.
