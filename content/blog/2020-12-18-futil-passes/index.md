...
For some `cell` definitions, we know statically the number of cycles it takes from the moment a `go` signal is raised to when the `done` signal is raised. For registers, this latency is a single cycle. Starting with this knowledge, it's possible to deduce the latencies of some `group`s. This pass targets `group`s where these conditions are true:
- All `go` ports are written to only by `done` ports or constants.
- All `done` ports are only written to by other `done` signals.
- The latency of each cell in this chain of `go`/`done` signals is known statically.

For example, this group, called `a`, takes only a single cycle to execute:
...
The `go` port of `register1` is driven by a constant, and the `done` signal of the group is written by the `done` signal of `register1`.
...
I added some simple unit tests to the FuTIL test suite, and also ran the pass on programs generated by a systolic array-to-FuTIL compiler, which does not insert static timing annotations. The generated programs continued to pass correctness tests, providing some confidence that the inference pass implementation is correct.
