...
A straightforward implementation of VLIW compilation at the basic block level can
...
out on optimization opportunities. This approach is refered to as "local compaction".
...
The core of this algorithm is inspired largely from [Trace Scheduling: A Technique for Global Microcode Compaction](https://people.eecs.berkeley.edu/~kubitron/courses/cs252-S12/handouts/papers/TraceScheduling.pdf). We simplified the algorithm to only allow entry into the top of the superblock so that we didn't have to worry about patching the program in subtle ways.
...
that have no predecessors in the graph. We do the following in a loop until we have scheduled all the instructions:
- Make an bundle.
...
The algorithm schedules an instruction only after all of its predecessors in the
...
### Generating Superblocks

The superblock algorithm generates a sequence of instructions and a DAG that are
...
The core insight with the superblock algorithm is that adding the DAG constraint that
