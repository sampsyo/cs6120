...
[^1]: After presenting on this, I realized the issue arrives from pseudo-SSA IRs. When the program is truly in IR, the larger of the two bugs (related to distant control flow not being respected), does not occur. However, if the program stores to a value and loads the value multiple times, we can get this bug.
