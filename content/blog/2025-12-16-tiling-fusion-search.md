        ...
        for n0 in [0, N/128):
            ...
            for k in [0, K):
                ...
                for m1 in [0, 128):
                    ...
                    for n1 in [0, 128):
                        C_local[m1, n1] += A[0, 0] * B_local[0, n1]
...
We leveraged this to build a formulation for fusing multiple einsums [here](https://github.com/natetyoung/tensor-program-dse/blob/main/cb_multifused_cpsat97.py). This formulation chooses the tiling for every einsum in the chain simultaneously, subject to the constraint that operands passed through fusion always take the outermost blocking level, so they all share exactly one bucket and can be fused like so:

    for m0 in [0, M0): # shared factors
        for n0 in [0, N0):
            ...
                einsum0
                einsum1
...
This "full-tree" approach is [here](https://github.com/natetyoung/tensor-program-dse/blob/main/full_tree_cpsat97.py). Rather than building orders of tiling levels in multiple einsums individually, each creating their own buckets, this CP-SAT formulation creates a tree structure among all the operands in the entire tensor program. Instead of constraints on "factors outside this tiling level," this program has constraints on "factors which are ancestors of this tiling level in the tree," and so on. In effect, this CP-SAT program synthesizes a restricted form of a [schedule tree](https://acohen.gitlabpages.inria.fr/impact/impact2014/papers/impact2014-verdoolaege.pdf) to create an optimal tiling and fusion for an input chain of einsums.
