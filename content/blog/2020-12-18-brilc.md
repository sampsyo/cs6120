...
* When to collect? Certainly this decision will ideally be informed by dynamic
  ...
  Without hardware or VM support, we may be limited in the choices of "hooks"
  into the code to even choose from. For example, to be a drop-in replacement for
  manual memory managment in C, one (naively at least) has only malloc calls to
  work with as possible collection times.
