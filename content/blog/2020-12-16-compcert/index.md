...
CompCert can generate assembly for PowerPC in 32-bit mode. Today, it can also generate assembly for ARM v6+ with the VFP coprocessor for floating-point arithmetic, as well as for Intel (AMD) in 32-bit mode with the SSE2 extension for integer-vector operations. 
...
The __eleventh pass__ executes common subexpression elimination, although it's not performed on function calls and it doesn't consider `x + y` and `y + x` equivalent. 

The __twelfth pass__ is dead code elimination. This may make a nonterminating program terminate. For example, suppose a variable is assigned to an integer divided by zero, but that variable is never used. This pass will turn the variable assignment into a `nop` with the justification that the source program was unsound.

The __thirteenth pass__ removes unused global variables. 

The __fourteenth pass__ takes the optimized RTL CFG and transforms it into an [LTL][ltl] CFG, which uses machine registers and stack slots and has basic blocks as nodes instead of individual instructions. Register allocation is done via the [iterated register coalescing algorithm][irca]. The algorithm is implemented in OCaml; if it succeeds, the LTL AST is [verified correct][vregalloc] with respect to the original function.
...
CompCert promises that it will not mistranslate code it compiles. Specifically, the executable it generates will not perform any action that the source code could not perform. 
...
A *bisimulation* relation between the source and target programs is a relation $R$ with the following conditions: 
...
Let $\mathcal{C}$ be a compiler, and $\sim$ a simulation as above. To be correct, $\mathcal{C}$ must promise $\mathcal{C}(s) = t \rightarrow s \sim t$ for all source programs $s$. Note that a compilation failure means that this theorem doesn't hold, since there's no $t$. 

### CompCert Approaches to Verified Compilation

Reasoning about the source code of a compiler is intuitively straightforward, but difficult to verify and inefficient in practice. Alternatively, an unverified compiler may be paired with a *translation validator* $\mathcal{V}$, which halts compilation if translation validation fails. Our promise then becomes: $\mathcal{C}(s) = t \land \mathcal{V}(t) = \texttt{true} \rightarrow s \equiv t$ for all source programs $s$. If the validator correctly validates results, then we have a verified compilation process. 

### Limitations

CompCert doesn't guarantee that the executables it generates simulate the input C99 program; C99 and the executable itself lack formal semantics. In reality, CompCert verifies that its compilation from its semantics for C99, *CSem*, to its semantics for assembly languages, *Asm*, preserves those semantics. 
...
Each pass between Csem and the abstract assembly language is proved with a forward simulation, some with a measure, some with direct transition correspondences. 
...
Airbus, an aerospace corporation, uses CompCert to write the software for its planes. A [2014 report][airbus] from the company indicated that the worst-case execution time of its programs had improved by 12%. The commercial success of CompCert speaks to its ability to hold its own as an optimizing compiler, although it still doesn't employ some more aggressive code optimizations. 
...
* In some sense, verifying software becomes a "turtles all the way down" problem where different tools must be assumed correct (or in need of verification). For instance, were a bug to be found in Coq, the correctness of CompCert would be meaningless. Does this assessment of the work resonate with you? Do you think it's unfair to characterize compiler correctness efforts this way? What makes CompCert valuable if we don't assume that the tools used are correct, if anything?
* Suppose you were asked to verify the full functionality of LLVM, fixing correctness bugs as needed but not significantly refactoring the implementation. You can't employ translation validation as a tactic. Would you augment CompCert with proofs of LLVM's optimizations, or would you scorch the earth and verify a new language akin to LLVM? 
