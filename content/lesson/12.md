...
You do not need to support Bril's [memory extension][mem], which makes things more complicated because it doesn't get automatically rolled back on speculation aborts.
...
* Check that you didn't break programs. For at least one benchmark (and ideally a few), create multiple inputs to the program that result in different outputs. Use one input to generate the trace and optimize the program, and use other inputs to check correctness. This guards against cases where your tracing optimization "overfits" and you end up with code that only works on one input.
