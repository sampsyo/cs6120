...
For example, imagine we are trying to devirtualize the method call `o.m()` in this program, where `Bar` and `Baz` are subclasses of `Foo` and both have different overrides of `m`:

    Foo o;  // a global variable
...
If we ask some kind of interprocedural analysis, *what run-time type does `o` have in the call `o.m()`?*, a correct analysis might say, *it might be either `Bar` or `Baz`*.
